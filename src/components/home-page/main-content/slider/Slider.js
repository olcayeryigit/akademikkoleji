...
const Slider = () => {
    ...
    return (
        ...
        fadeEffect={{ crossFade: true }}
            ...
            className='container  w-screen h-[640px] md:h-[480px] md:h-full'
        >
...
<h2 className='pt-4 md:pt-0 text-lg md:text-xl text-gray-200 tracking-wide mb-2 md:mb-6'>
    Kids ve Teens Yaş Gruplarına Özgü Nitelik Kazandıran Dil Eğitimi
</h2>


                    <div className='diller flex flex-col  md:flex-row gap-6 md:gap-8 '>
                        {visibleLanguages.map((lang, index) => (
                            <div
                                key={index}
                                className={`pt-3 md:pt-0 flex  flex-row md:flex-col gap-4 items-center transition-opacity duration-500 ease-in-out`} 
            ...
            <SwiperSlide className='relative p-0 md:ps-16 pt-8  md:pt-28'>
    {/* Arka Plan Görseli */}
    ...
    <div className='relative z-10 flex flex-col '>
        ...
        <h1
            ...
            className='text-xl md:text-3xl font-extrabold text-white  mb-4 text-center'
        >
        ...
        <h2 className='pt-6 md:pt-0 text-lg md:text-lg text-gray-200  mb-6 text-center'>
            Öğrencilerimizi Yapay Zeka (AI), Robotik ve Kodlama Eğitimleri ile Geleceğe Hazırlıyoruz!
        </h2>

        {/* Resimler için satır */}
        <div className='flex flex-row flex-wrap justify-center gap-3 pt-8 md:pt-0'>
            {informatics.map((inf, index) => (
                <div key={index} className='relative mb-8 d:mb-0 flex flex-col  items-center lg:w-36 '>
                    <div 
                    ...
                    <h1 className='text-center text-sm md:text-lg font-semibold text-white '>
                        {inf.name}
                    </h1>
                </div>
            ))}
