...
<button
  key={item.id}
  className={`flex items-center gap-3 px-8 py-4 rounded-full text-sm font-semibold transition-all duration-300 shadow-lg transform hover:scale-105 ${
    selectedId === item.id
      ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-xl"
      : "bg-orange-200 text-orange-800 hover:bg-orange-300 hover:shadow-md"
  }`}
  onClick={() => handleButtonClick(item.id)}
>
  <FaQuestion
    className={`transition-colors ${
      selectedId === item.id ? "text-white" : "text-orange-700 hover:text-white"
    }`}
  />
